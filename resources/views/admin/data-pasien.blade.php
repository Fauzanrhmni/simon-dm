...
<section
    ...
    <div class="w-full mt-14 flex flex-row justify-between">
        <form method="GET" action="{{ route('admin.data-pasien') }}" class="w-[70%] bg-white rounded-xl">
            <input type="text" name="search" id="search" placeholder="Cari Nama Pasien" value="{{ request('search') }}"
                class="w-full rounded-xl border-none placeholder-slate-400 placeholder:text-xs text-xs focus:outline-none focus:border-[#0BB4A6] focus:ring-[#0BB4A6] py-2.5">
        </form>
